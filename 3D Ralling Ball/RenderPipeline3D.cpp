...
IRenderPipeline3D::IRenderPipeline3D()
{
	...
	m_pVB_Rasterized = new std::vector<RasterizedFragment>;//这些就是一堆指针
	m_pTexture = nullptr;//wait for user to set空指针
	mWorldMatrix.Identity();//使矩阵单位化
	mViewMatrix.Identity();
	mProjMatrix.Identity();//Identity就是单位化
	mTexCoord_offsetX=0.0f;
	mTexCoord_offsetY=0.0f;
	mTexCoord_scale=1.0f;
	mCameraPos = { 0,0,0 };//照相机位置
	mLightEnabled = TRUE;
	for (UINT i = 0;i < c_maxLightCount;++i)memset(&mDirLight[i],0,sizeof(DirectionalLight));//初始化为零
}

BOOL IRenderPipeline3D::Init(RenderPipeline_InitData & initData)//将一个RenderPipeline_InitData的值赋给这个类
{
...
{
	//clear the last Draw Call ruins...清除
	m_pVB_HomoSpace->clear();
	m_pVB_HomoSpace_Clipped->clear();
	m_pIB_HomoSpace_Clipped->clear();////清空向量中的元素
	m_pVB_Rasterized->clear();

	//..............
	UINT offset = drawCallData.offset;//offset应该是起点
	UINT vCount = drawCallData.VertexCount;
	auto const pVB = drawCallData.pVertexBuffer;//the data the ptr point to can't be modified const表示不能修改
	auto const pIB = drawCallData.pIndexBuffer;//

	//reserve space for vector, because they need to push_back later
	m_pVB_HomoSpace->reserve(vCount);//保留适当的容量。
	...
	//--------------------HOMO SPACE CLIPPING-------------------
	HomoSpaceClipping_Triangles(pIB);//三角形裁剪

	//------------------------------RASTERIZER-----------------------------
	RasterizeTriangles();//光栅化
	
	//------------------------------PIXEL SHADER-----------------------------
	for (auto& rasterizedVertex: *m_pVB_Rasterized)
	{
		
		PixelShader_DrawTriangles(rasterizedVertex);//逐像素处理
	}
...
{
	VertexShaderOutput_Vertex outVertex;//输出的投影结构

	//vertex position
	VECTOR4 pos(inVertex.pos.x, inVertex.pos.y, inVertex.pos.z, 1.0f);
	pos = Matrix_Multiply(mWorldMatrix, pos);//转化为世界坐标系下的坐标
	pos = Matrix_Multiply(mViewMatrix, pos);//转化为眼坐标系下的坐标

	//non-linear operation
	float Z_ViewSpace = pos.z;//存储眼坐标系下的z坐标，用于阴影的计算
	...
	//Normal also need transformation,actually it need a World-Inverse-Transpose世界逆转置
	//But I rule out all non-Orthogonal（非正交转化） Transformation, so inverse（反向）
	//can just use Transpose to substitude
...
void IRenderPipeline3D::HomoSpaceClipping_Triangles(std::vector<UINT>* const pIB)
{
	...
	*m_pVB_HomoSpace_Clipped = std::move(*m_pVB_HomoSpace);//就把move当成复制的意思
	...
	while(i < m_pIB_HomoSpace_Clipped->size()-3)
	{
		...
		bool b1 = v1.posH.x <= -1.0f || v1.posH.x >= 1.0f ||//为什么是一？？？？
		...
		//violently and roughly clip triangles ::: only when 3 vertices out of boundary如果越界就删除
		...
		if (b1 && b2 && b3==true)
		{
			...
			for (UINT j = 0;j < 3;j++)m_pIB_HomoSpace_Clipped->pop_back();//删除最后三个数据
		}
		else
		{
...
void IRenderPipeline3D::HomoSpaceClipping_Points(std::vector<UINT>* pIB)//三角形裁剪
{
		//copy VB/IB into new list, then erase (or std::remove() );
	*m_pIB_HomoSpace_Clipped = (*pIB);
...
void IRenderPipeline3D::RasterizeTriangles()//光栅化
{
	...
	for (UINT tri = 0;tri < m_pIB_HomoSpace_Clipped->size() - 2;tri += 3)
	{
		...
		auto convertToPixelSpace = [&](const VertexShaderOutput_Vertex& v, VECTOR2& outV)
		{
			...
			outV.y = float(mBufferHeight) * (-v.posH.y + 1.0f) / 2.0f;//这个公式什么意思？？？
		};
		...
		//a determinant（行列式） to solve B-Lerp Coord equation
		//refer to doc for more math detail.
		float D = basisVector1.x*basisVector2.y - basisVector2.x*basisVector1.y;//行列式的值

		//in such circumstances,A B C lie on the same line.
		//------well , use THE SIGN OF D can implement BACKFACE CULLING--------
		if (D == 0)return;//判断是否平行，若平行不是三角形
		...
		//------------ horizontal scan line Intersection ------------水平扫描法求交
...
void IRenderPipeline3D::RasterizePoints()
{
	for (UINT i = 0;i < m_pIB_HomoSpace_Clipped->size() ;++i)
	{
		...
		if (mFunction_DepthTest(UINT(v1_pixel.x), UINT(v1_pixel.y), depth) == FALSE)continue;
...
BOOL IRenderPipeline3D::mFunction_HorizontalIntersect(float y, const VECTOR2 & v1, const VECTOR2 & v2, 
const VECTOR2 & v3, UINT & outX1, UINT & outX2)
{
...
VECTOR4 IRenderPipeline3D::mFunction_VertexLighting//对顶点的进行光照处理，获取它漫射光，反射光，
(const VECTOR3& vPosW, const VECTOR3& vNormalW)
{
	...
	for (UINT i = 0;i < c_maxLightCount;++i)
	{
		if (mDirLight[i].mIsEnabled == TRUE)
		{
			...
			VECTOR3 toEye = mCameraPos - vPosW;//从原点到摄像机位置的向量
			toEye.Normalize();
			...
			//diffuse Factor (first make sure that angle <normal,light> 
			//is less than PI/2首先确保法线与光线的夹角小于90度
