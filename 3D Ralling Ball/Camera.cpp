...
ICamera::ICamera()
{
	...
	mRotateY_Yaw=0;
	mRotateZ_Roll=0;
	mViewAngleY = 60.0f/180.0f * CONST_PI;//视角为60度
	mAspectRatio = 1;
	...
	mNearPlane=1.0f;
	mFarPlane= 3000.0f;
	//NearPlaneDistance表示Camera的近端裁减，相当于Camera的盲区，当Camera和三维物品的距离小于这个值时，就不显示了；
	//FarPlaneDistance和NearPlaneDistance相反，当Camera和三维物品的距离大于FarPlaneDistance时，就不显示。

	mMatrixProjection= Matrix_PerspectiveProjection(mViewAngleY,mAspectRatio,mNearPlane,mFarPlane);
	//透视矩阵的求解，需要带入视角，比例大小，近景和远景
...
{
	//roll翻滚不需要更新lookat
	mRotateZ_Roll = AngleZ;//前进的时候看的东西没有改变
}
...
void	ICamera::mFunction_UpdateRotation()
{
	...
	//检查新direction是否为0
	tmpDirection=mLookat-mPosition;//lookat是看到的东西所在的位置，position是照相机所在的位置，
								   //方向就是看到的东西减去摄像机所在位置
