...
namespace Math
{
	const float CONST_PI = 3.1415926f;//圆周率

	struct VECTOR2//定义一个储存二维坐标的结构
	{
		VECTOR2() { };
		VECTOR2(float X, float Y) { x = X; y = Y; };//构造函数
		VECTOR2(const float column[2])//用数组传递数据
		{
			x = column[0];
			y = column[1];
		}
		...
		BOOL	operator==(const VECTOR2& vec)//重载相等关系运算符
		{
			if (vec.x == x&&vec.y == y)
				return TRUE;
			else
		...
		VECTOR2& operator+=(const VECTOR2& vec)//加等
		{
			x += vec.x;
			y += vec.y;
			return *this;
	...
	struct MATRIX4x4//一个4*4的矩阵
	{
		MATRIX4x4()
		{
			Identity();//默认构造函数，先将其单位化
		}
	...
	struct BOUNDINGBOX//定义包含两个vector3 的结构，并将其初始化为1，1，1，和0，0，0，
	{
		BOUNDINGBOX()
		{
			max = { 1,1,1 };
	...
	extern float Vec2_Dot(const VECTOR2& vec1, const VECTOR2& vec2);//extern表示声明函数名，使得其他文件可以访问此声明
