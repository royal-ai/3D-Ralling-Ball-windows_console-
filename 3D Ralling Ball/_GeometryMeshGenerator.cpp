...
/***********************************************************************
					...
					Class : Geometry Mesh Generator//几何网格生成器
...
void IGeometryMeshGenerator::CreatePlane
(float fWidth, float fDepth, UINT iRowCount, UINT iColumnCount, 
	std::vector<Vertex>& outVerticeList, std::vector<UINT>& outIndicesList)// 创建一个平面
{
	mFunction_Build_A_Quad(
		VECTOR3(-fWidth / 2, 0, fDepth / 2),
		VECTOR3(fWidth / (float)(iColumnCount - 1), 0, 0),//tangent切线？？
...
void IGeometryMeshGenerator::CreateBox(float fWidth, float fHeight, float fDepth, 
	UINT iDepthStep, UINT iWidthStep, UINT iHeightStep, std::vector<Vertex>& outVerticeList,
	std::vector<UINT>& outIndicesList)
{
	/*
	Y  |
	|      /  Z
	...
	*/
	...
	tmpBaseIndex = 0;
	mFunction_Build_A_Quad(
		VECTOR3(-fWidth / 2, -fHeight / 2, -fDepth / 2),//创建盒子的底面由左下角的原点出发，因为都是负的
	...
	mFunction_Build_A_Quad(
		VECTOR3(-fWidth / 2, fHeight / 2, fDepth / 2),//同理
		VECTOR3(tmpStep1, 0, 0),
		VECTOR3(0, 0, -tmpStep2),
		iDepthStep,
		iWidthStep,
...
void 
IGeometryMeshGenerator::CreateSphere(float fRadius, UINT iColumnCount, //创建一个球咯
	UINT iRingCount, BOOL bInvertNormal, std::vector<Vertex>& outVerticeList, std::vector<UINT>& outIndicesList)
{


	//iColunmCount : Slices of Columns (Cut up the ball Vertically)垂直切面得到的圆的数量
	//iRingCount: Slices of Horizontal Rings (Cut up the ball Horizontally)水平切面
	//the "+2" refers to the TOP/BOTTOM vertex
	//the TOP/BOTTOM vertex will be restored in the last 2 position in this array
	//the first column will be duplicated复制 to achieve adequate texture mapping
	VECTOR3* tmpV;
	VECTOR2* tmpTexCoord;
	UINT tmpVertexCount = (iColumnCount+1) * iRingCount +2;//the "+2" refers to the TOP/BOTTOM vertex
...
void IGeometryMeshGenerator::CreateCylinder
(float fRadius, float fHeight, UINT iColumnCount, UINT iRingCount,
	std::vector<Vertex>& outVerticeList, std::vector<UINT>& outIndicesList)//圆柱？？？
{
...
inline void IGeometryMeshGenerator::mFunction_Build_A_Quad//应该是创建一个面的意思
(VECTOR3 vOriginPoint, VECTOR3 vBasisVector1, VECTOR3 vBasisVector2, 
	UINT StepCount1, UINT StepCount2, UINT iBaseIndex, 
	std::vector<Vertex>& outVerticeList, std::vector<UINT>& outIndicesList)
{
	VECTOR3 tmpNormal;//法向量
	Vertex tmpCompleteV;
	tmpNormal=Vec3_Cross( vBasisVector1, vBasisVector2);//两个向量的叉乘，因为法向量与两个基础向量垂直
	tmpNormal.Normalize();
